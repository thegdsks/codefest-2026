...
# ---------------------------------------------------------------------------
# Helper: fire a single curl request, print status, return http_code + body
# ---------------------------------------------------------------------------
_curl_capture() {
  local method="$1"
  local url="$2"
  ...
  local curl_args=(-s -o "$tmp_body" -w "%{http_code}" -X "$method" --max-time 15)
  ...
  if echo "$raw_body" | jq . &>/dev/null; then
    ...
    LAST_BODY="$(jq -n --arg b "$raw_body" '{"raw":$b}')"
  fi
}
...
capture_login_success() {
  ...
  _curl_capture POST "${API_URL}/auth/login" \
    "$AUTH_HEADER" \
  ...
  # The -d flag above is in extra_headers which curl_capture passes via -H,
  # which is wrong for -d. Redo with explicit curl to pass body correctly.
  local tmp_body
  tmp_body=$(mktemp)
...
capture_auth_session() {
  ...
  if [[ -z "$BEARER_TOKEN" ]]; then
    ...
    jq -n '{capturedAt:"skipped", reason:"no_bearer_token"}' \
...
capture_mfa_flow() {
  ...
  if [[ $curl_exit -ne 0 ]]; then
    ...
    jq -n '{capturedAt:"skipped", reason:"mfa_trigger_login_curl_error"}' \
...
capture_logout() {
  ...
  if [[ -z "$BEARER_TOKEN" ]]; then
    ...
    jq -n '{capturedAt:"skipped", reason:"no_bearer_token", httpStatus:"n/a", note:"logout requires a valid bearer token"}' \
