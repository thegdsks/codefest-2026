...
if (!DRY_RUN) {
  const missing = ['DEMO_USER', 'DEMO_PASSWORD', 'DEMO_MFA_OTP'].filter(
    (k) => !process.env[k]
  );
  if (missing.length > 0) {
...
await step('POST /auth/login -> challengeId', async () => {
  ...
  if (!challengeId) {
    ...
    if (!token) throw new Error('No challengeId or token in login response: ' + JSON.stringify(body));
    bearerToken = token;
  }
});
...
await step('POST /admin/decisions/{id}/release -> 200', async () => {
  ...
  const res = await fetchWithTimeout(
    `${BASE_URL}/admin/decisions/${targetDecisionId}/release`,
    {
      method: 'POST',
      headers: { Authorization: basicAuth(DEMO_USER, DEMO_PASSWORD) },
    }
...
await step('GET /admin/sessions -> current session present', async () => {
  ...
  if (!found) {
    throw new Error(
      `No session found for userId=${DEMO_USER} in ${sessions.length} session(s)`
    );
  }
});
...
function printSummary() {
  ...
  console.log(
    `${'Step'.padEnd(52)} ${'Duration'.padStart(10)}  Status`
  );
