...
export default function Skeleton({ className = 'h-4 w-full', rows = 1 }: SkeletonProps) {
  if (rows === 1) {
    return <div className={`motion-safe:animate-pulse rounded bg-zinc-800 ${className}`} />;
  }
  const count = Math.min(Math.max(rows, 1), ROW_KEYS.length);
  return (
    <div className="space-y-2">
      {ROW_KEYS.slice(0, count).map((k) => (
        <div key={k} className={`motion-safe:animate-pulse rounded bg-zinc-800 ${className}`} />
      ))}
    </div>
  );
}
