...
// 4px left border color keyed by action.
...
type TransferRow = DecisionRow & {
  amount?: number | string;
  recipientId?: string;
  velocityCount?: number;
};
type EngagementRow = DecisionRow & { signal?: string; target?: string; surface?: string };

function loginSubline(row: DecisionRow): string {
...
function transferSubline(row: DecisionRow): string {
...
function engagementSubline(row: DecisionRow): string {
  const r = row as EngagementRow;
...
function typeSubline(row: DecisionRow): string {
  ...
  if (t === 'FRAUD_LOGIN' || t === 'LOGIN') return loginSubline(row);
  if (t === 'FRAUD_TRANSFER' || t === 'TRANSFER') return transferSubline(row);
  if (ENGAGEMENT_TYPES.has(t)) return engagementSubline(row);
  if (t === 'MFA_VERIFY' || t === 'MFA_EVENT') {
    const path = (row as DecisionRow & { mfaPath?: string }).mfaPath ?? row.reasonCode ?? '';
    return path ? `MFA via ${path}` : reasonOf(row);
  }
  if (t === 'DECISION_RELEASE') {
...
function InlineExpand({
  ...
  return (
    <div className="border-t border-[color:var(--border)] bg-[color:var(--bg-surface)]/60 px-4 py-3 pl-10">
      {explanation ? (
...
// ---------------------------------------------------------------------------
// Group row (collapses multiple identical back-to-back decisions)
// ---------------------------------------------------------------------------
...
function GroupRow({
  ...
  return (
    <>
      {/* Badge strip + representative row */}
      <div className="relative">
        <div className="flex items-center gap-1 border-b border-[color:var(--border)]/50 bg-[color:var(--bg-surface)]/20 px-3 py-0.5">
          <span className="text-[10px] text-[color:var(--text-dim)]">Grouped:</span>
          <GroupChip
            count={group.count}
