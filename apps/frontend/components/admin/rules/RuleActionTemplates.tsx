...
function SurfaceMock({ mock, copy }: SurfaceMockProps) {
  if (mock === 'tooltip') {
    return (
      <div className="flex h-14 items-center justify-center rounded-md border border-[color:var(--border)] bg-[color:var(--bg)]/80 px-2">
        <span className="inline-flex max-w-full items-center gap-1.5 truncate rounded-full bg-[color:var(--bg-elevated)] px-2.5 py-1 text-[10px] text-[color:var(--text)] shadow-sm">
          {copy}
        </span>
      </div>
    );
  ...
  if (mock === 'banner') {
    return (
      <div className="flex h-14 items-center rounded-md border border-[color:var(--border)] bg-[color:var(--bg)]/80 p-1.5">
        <div className="flex h-full w-full items-center overflow-hidden rounded bg-[color:var(--bg-surface)]">
          <div className="h-full w-1 shrink-0 bg-indigo-500" />
          <span className="truncate px-2 text-[10px] text-[color:var(--text)]">{copy}</span>
        </div>
      </div>
    );
  }
  if (mock === 'modal') {
    return (
      <div className="flex h-14 items-center justify-center rounded-md border border-[color:var(--border)] bg-[color:var(--bg)]/80 p-1.5">
        <div className="flex h-full w-3/4 flex-col overflow-hidden rounded border border-[color:var(--border-strong)] bg-[color:var(--bg-surface)] shadow-md">
          <div className="flex h-3 items-center gap-1 border-b border-[color:var(--border)] bg-[color:var(--bg)] px-1.5">
            <span className="size-1 rounded-full bg-zinc-700" />
            <span className="size-1 rounded-full bg-zinc-700" />
          </div>
          <div className="flex flex-1 items-center justify-center px-1.5">
            <span className="truncate text-[9px] text-[color:var(--text-muted)]">{copy}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-14 items-center justify-center rounded-md border border-dashed border-[color:var(--border-strong)] bg-[color:var(--bg)]/40 text-[10px] text-[color:var(--text-dim)]">
      Build your own
    </div>
  );
}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {TEMPLATES.map((t) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`relative text-left rounded-xl border bg-[color:var(--bg-surface)]/40 p-3 transition-colors hover:bg-[color:var(--bg-surface)]/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                active
                  ? 'border-indigo-500/50 ring-2 ring-indigo-400/70'
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-[color:var(--text)]">{t.title}</span>
              </div>
              <div className="mt-1 text-[11px] leading-snug text-[color:var(--text-dim)]">
                {t.description}
              </div>
              <div className="mt-3">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-[color:var(--text)]">{CUSTOM.title}</span>
          </div>
          <div className="mt-1 text-[11px] leading-snug text-[color:var(--text-dim)]">
            {CUSTOM.description}
          </div>
