...
| (SPA simulator    |<------>| (Lambda + DDB +   |<------>| dashboard         |
|  for the demo)    |   API  |  Bedrock Haiku)   |   API  | (SPA /admin)      |
+-------------------+        +-------------------+        +-------------------+
                                       ^
                                       |
                              activity stream
...
Three apps, one repo. The customer surface and the admin console share the same SPA for the demo. The decision engine is a single Lambda behind API Gateway. In production the engine splits into hot / warm / cold lanes by decision complexity (rules, LLM, batch). See [`docs/architecture.md`](./docs/architecture.md) for the scaled architecture and cost model.
