...
var EngagementContext = react.createContext(null);
...
function EngagementProvider({
  ...
  const [currentIntervention, setCurrentIntervention] = react.useState(null);
  ...
  const trackSearch = react.useCallback((query) => {
    captureRef.current?.trackSearch(query);
  }, []);
  const trackHealthyEvent = react.useCallback((eventKey) => {
    clientRef.current?.trackHealthyEvent(eventKey);
  }, []);
  const getTrustScore = react.useCallback(() => {
    return clientRef.current?.getTrustScore() ?? 70;
  }, []);
  const setFlowState = react.useCallback((state) => {
  ...
  function renderSurface() {
    ...
    if (CustomComponent) {
      return /* @__PURE__ */ jsxRuntime.jsx(CustomComponent, { intervention: currentIntervention, onDismiss: dismiss });
    }
    if (surfaceType === "nudge_banner") {
      return /* @__PURE__ */ jsxRuntime.jsx(NudgeBanner, { intervention: currentIntervention, onDismiss: dismiss });
    }
    if (surfaceType === "offer_modal") {
      return /* @__PURE__ */ jsxRuntime.jsx(OfferModal, { intervention: currentIntervention, onDismiss: dismiss });
    }
    if (surfaceType === "help_tooltip") {
      return /* @__PURE__ */ jsxRuntime.jsx(HelpTooltip, { intervention: currentIntervention, onDismiss: dismiss });
    }
    return null;
  }
...
exports.EngagementProvider = EngagementProvider;
exports.useEngagement = useEngagement;
